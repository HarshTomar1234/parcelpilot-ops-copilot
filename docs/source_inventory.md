...
`assessment-brief/CalQuity AI Engineer â€” Job Description & AI Agent Assessment.docx`

Not a system input â€” it is the requirement spec. Key constraints it imposes:
...
| `SRC-05` | 05_Northstar_Logistics_Enterprise_Agreement.pdf | agreement | ACTIVE | 2026-01-01 â†’ 2026-12-31 | `ACCT-001` only | `AGREEMENT` |
| `SRC-06` | 06_LumenWorks_Service_Agreement.pdf | agreement | ACTIVE | 2026-03-01 â†’ 2027-02-28 | `ACCT-002` only | `AGREEMENT` |
| `SRC-07` | ParcelPilot_Assessment_Data.xlsx | structured_data | CURRENT | snapshot 2026-08-16 11:00 IST | all accounts | `STRUCTURED` |

### SRC-01 â€” Support Policy v3 (CURRENT)

Effective 1 May 2026, supersedes v2.

- **Â§1 Scope and source precedence** â€” states the precedence chain the whole
  system is built on: signed customer agreement â†’ current support policy â†’
...
- **Â§2 Severity definitions** â€” P1 Critical (complete production outage
...
- **Â§3 Default first-response targets** â€” Enterprise 30 min 24x7 / 2 h / 1
...
- **Â§4 Escalation** â€” P1 escalates immediately. If a target is already breached,
...
### SRC-02 â€” Support Policy v2 (DEPRECATED)
...
### SRC-03 â€” Cancellation & Service Credit SOP v4 (CURRENT)

Effective 15 June 2026.

- **Â§1 Order cancellation** â€” DRAFT free; BOOKED-not-yet-PICKED_UP free within
  ...
  cannot be cancelled.
- **Â§2 Failed-pickup service credits** â€” default eligibility: pickup more than
...
- **Â§3 Approval and uncertainty** â€” any individual credit above INR 1,000 needs
...
Â§3 is effectively a business requirement for the confirmation workflow.

### SRC-04 â€” Product Operations Guide & Known Issues (CURRENT)

Updated 14 August 2026.

- **Â§1 Plan capabilities** â€” Bulk Upload on Growth and Enterprise, up to
  **5,000 rows** per CSV; Standard has no Bulk Upload. BOOKED = created, pickup
  not yet confirmed; PICKED_UP = pickup confirmed.
- **Â§2 Known issues** â€” `KI-208` Bulk Upload failures above ~3,000 rows
...
- **Â§3 Resolved issue** â€” `KI-176` address validation, resolved 18 July 2026;
...
### SRC-05 â€” Northstar Logistics Enterprise Agreement

Account `ACCT-001`, term 1 Jan 2026 â†’ 31 Dec 2026, ACTIVE.

- **Â§1 Support terms** â€” replaces standard targets: P1 15 minutes 24x7, P2 1 hour, P3 8 business hours.
- **Â§2 Shipment cancellation** â€” may cancel **any** BOOKED shipment before
  pickup with **no fee, regardless of how long ago it was booked**. Once
  PICKED_UP, standard return-to-origin applies.
- **Â§3 Service credits** â€” monthly aggregate credits capped at **INR 5,000**;
  otherwise the current SOP applies.
- **Â§4** â€” dedicated CSM Priya Mehta.

**Scope caveat:** Â§3 caps the aggregate but does **not** replace the SOP's 2-hour
threshold or credit formula. Do not generalize the Â§2 cancellation waiver into a
credit waiver.

### SRC-06 â€” LumenWorks Service Agreement

Account `ACCT-002`, Growth plan, term 1 Mar 2026 â†’ 28 Feb 2027, ACTIVE.

- **Â§1 Support terms** â€” P1 2 business hours, P2 4 business hours, P3 2 business
  days, and **no weekend or after-hours support coverage**.
- **Â§2 Cancellation terms** â€” explicitly **no** fee waiver; use the current SOP.
- **Â§3 Failed-pickup credits** â€” more than **4 hours** past window end + carrier
  fault + no customer fault â†’ fixed **INR 300**. This clause explicitly replaces
  the SOP's default amount **and** timing threshold.

**Scope caveat:** Â§3 replaces the threshold in *both* directions. A 3-hour
carrier-fault delay earns a credit under the default SOP but **nothing** for
LumenWorks. Â§2 means an agreement can also *confirm* the default rather than override it.

### SRC-07 â€” ParcelPilot_Assessment_Data.xlsx

Four sheets: `README`, `accounts`, `orders`, `tickets`. Full schema in
[`data_dictionary.md`](data_dictionary.md).

- **Snapshot: `2026-08-16 11:00 Asia/Kolkata`** â€” authoritative reference time.
...
- `accounts.contract_file` is empty for `ACCT-003` and `ACCT-004`, matching the
  ...
  standard Enterprise policy applies â€” a useful contrast against `ACCT-001`.
